...
// @icon        https://cdn.jsdelivr.net/gh/EhTagTranslation/EhSyringe@f7ba93f0f4de93250deacde90fdad4e25b724524/src/assets/logo.svg
...
var parent = __webpack_require__(3400);
...
var parent = __webpack_require__(9869);
...
var parent = __webpack_require__(4378);
// TODO: Remove from `core-js@4`
...
var parent = __webpack_require__(9697);
...
var parent = __webpack_require__(9519);
...
var parent = __webpack_require__(2210);
// TODO: remove from `core-js@4`
...
var getVirtual = __webpack_require__(8770);
...
var $ = __webpack_require__(2109);
var fromAsync = __webpack_require__(3253);
...
var toIndexedObject = __webpack_require__(5656);
var arraySlice = __webpack_require__(206);
var arrayToSpliced = __webpack_require__(1654);
var addToUnscopables = __webpack_require__(1223);
...
/* harmony default export */ var dist = (idbReady);
...
var Introduce = /*#__PURE__*/function () {
  ...
  _createClass(Introduce, [{
    ...
    value: function onImageError(ev) {
      var target = ev.target; // eslint-disable-next-line @typescript-eslint/unbound-method
    ...
    value: function () {
      var _openIntroduceBox = _asyncToGenerator( /*#__PURE__*/regenerator_default().mark(function _callee2(namespace, key, canceled) {
        ...
        var timer, tagData, editorUrl, _tagData$links;
            ...
            switch (_context2.prev = _context2.next) {
              ...
              case 8:
                ...
                this.introduceBox.querySelectorAll('img').forEach(function (img) {
                  img.referrerPolicy = 'no-referrer'; // eslint-disable-next-line @typescript-eslint/unbound-method
...
merge(/^\/bitcoin\.php/, undefined, {
  ...
  'Read this thread for more information about donating and how to buy coins': '阅读帖子，了解更多关于捐赠和如何购买虚拟货币的信息',
  ' Bitcoin (BTC)': ' 比特币（BTC）',
  ...
  ' when the old one is used. You will still be credited if you reuse recent addresses, but please use the currently displayed address whenever possible.': ' 。如果你重复使用之前的地址，仍然会被计入，但请尽可能使用当前显示的地址。',
  ...
  'You can use this form to send a private message from the gallery system with your wallet address. This guarantees that a given wallet address belongs to you and that it exists in the E-Hentai system.': '你可以使用这个表单，以画廊系统（Gallery System）的名义发送一条包含钱包地址的私信。这保证了该钱包地址属于你，并且存在于 E-Hentai 系统中。',
  'Member name: ': '收件人：',
  Send: '发送',
  ...
  ' until they are included in a block and reach two confirmations. This typically takes less than an hour.': '直到他们被纳入一个区块并达到两个确认。通常需要不到一个小时。',
  ...
  'Apply Slot': '我要领养',
  'The donated coins are applied as the equivalent value in USD at the time of your choosing, using an inflated rate which is': '捐赠的虚拟货币在您使用的时候会被等值的转换为美元，',
  'calculated from the recent average historic value. In order to minimize unexpected price fluctuations, the site rate will': '根据最近平均历史价格作为汇率，以减少意外的价格波动，',
  'never drop sharply, but adjusts slowly over time if the real-world rate is more than 10% below the site rate.': '如果现实世界的汇率比网站的低 10% 以上，网站的汇率不会急剧下降，会随着时间的推移缓慢调整。',
...
}, [[/Refunded (.*?) Hath/, '退还 $1 Hath'], [/^(\$.*?) Donation/, '捐赠 $1'], [/^A wallet address PM was sent to user (.+?) \(uid=(\d+)\)$/, '你的钱包地址已私信发送给 $1（UID $2）']]);
...
}, [[/^(\d+) times?$/, '$1 次'], [/Average: ([\d.]+)/, '平均值：$1'], [/Rate as ([\d.]+) stars?/, '$1 星'], [/Torrent Download \(\s*(\d+)\s*\)/, '种子下载（$1）'], [/^Posted on (.*?) by:\s*/, "\u8BC4\u8BBA\u65F6\u95F4\uFF1A$1 \xA0\u4F5C\u8005\uFF1A"], [/^Posted on (.*?)\s*/, "\u8BC4\u8BBA\u65F6\u95F4\uFF1A$1"], [/^, added (.*?)$/, "\uFF0C\u66F4\u65B0\u4E8E $1"], [/^There (is|are) ([\d,]+) more comments? below the viewing threshold - $/, '还有 $2 条评论尚未显示 - '], [/^This gallery is unavailable due to a copyright claim by (.*)\.$/, '由于 $1 提出的版权要求，这个画廊无法使用。']]);
...
merge(/^\/hathperks\.php/, undefined, {
  ...
  'If running H@H is not an option, you can exchange Credits for Hath at the ': '如果你不能运行 H@H，你可以在这里用 Credits 交换 Hath：',
...
merge(/^\/hentaiathome\.php\??$/, undefined, {
  ...
  ' Must be at least 10 GB. More is better, but space must be dedicated.': '至少为 10GB。越多越好，但空间必须专用。',
  'Speed Test': '速度测试结果',
  'Go to ': '前往 ',
  ...
  'If the auto-detected country is wrong, contact Tenboro to have an override applied. Include the correct country, client ID and IP address in your message, and make sure to keep your client running. Having the client set to the wrong country will make it perform worse than it would otherwise do.': '如果自动检测的国家/地区错误，请联系 Tenboro 以进行更改。在消息中包含正确的国家/地区、客户端 ID 和 IP 地址，并确保您的客户端保持运行。位于错误的国家/地区的客户端的性能表现将不如预期。',
  'Free Archive Quota: ': '免费的存档配额：',
  ', measured in a 168-hour sliding window. The cap is updated once every hour. Clients must have been running for more than 24 hours with a hitrate above 1 to qualify.': '，按照最近 168 小时的在线时间进行计算。上限每小时更新一次。客户端必须保持运行超过 24 小时且点击率高于 1 才能获得资格。',
...
merge(/^\/hentaiathome\.php\?.*act=settings/, undefined, {
  ...
  'Can be 443 (recommended) or most numbers between 1024 and 65534.': '可以使用 443（推荐）或 1024~65534 的大部分端口。',
  'This port must be opened in your firewall, and forwarded from any NAT-based cable/ADSL modems/routers you connect to the Internet through.': '该端口必须在您的防火墙中打开，并从您连接到 Internet 的任何基于 NAT 的电缆/ADSL 调制解调器/路由器转发。',
  'Note: The port cannot be changed while the client is running.': '注意：客户端运行时不能更改端口。',
  'Client Name': '客户端名称',
  'You can set a custom name for this client here. This will be used in the various listings this client appears in.': '您可以在此处为此客户端设置自定义名称。这将用于该客户端出现的各种列表中。',
  'Maximum Burst Speed': '最大并发速率',
  'Actual utilization will reach at most 80% of this over time, unless you also set an hourly limit below.': '随着时间的推移，实际利用率最多将达到该值的 80%，除非您还在下方设置了每小时限制。',
  'Turning off the client-side throttle saves some CPU and may make you test higher, but could make the traffic more bursty.': '关闭客户端侧速度限制可以节省一些 CPU 并可能使您的测试更高，但可能会使流量更加突发。',
  ' KBytes/s': ' KB/s',
  'Disable Client-Side Speed Limit': '禁用客户端侧速度限制',
  'Warning: 1 KB/s or KBps (Kilo Bytes per Second) is equivalent to 8 Kb/s or Kbps (Kilo Bits per Second). Internet speeds are typically advertised as the latter, so make sure you use the right one. You should not set this higher than the upstream bandwidth of your Internet connection.': '警告：1 KB/s 或 KBps（千字节每秒）相当于 8 Kb/s 或 Kbps（千比特每秒）。Internet 速度通常被宣传为后者，因此请确保使用正确的速度。您不应将此设置为高于 Internet 连接的上行带宽。',
  'Maximum Disk Cache Size': '最大磁盘缓存大小',
  'How much disk space you wish to reserve for this client. The reserved space must always be available.': '您希望为此客户端保留多少磁盘空间。 保留空间必须始终可用。',
  'Static Range allocation is limited to 1 per 250 MB of disk space.': '静态范围分配限制为每 250MB 磁盘空间 1 组。',
  ...
  'If you believe your client key has been compromised, you can reset the key by checking this box. You will have to re-enter the key the next the the client starts.': '如果您认为您的客户端 Key 已被泄露，您可以通过选中项来重置密钥。下次客户端启动时，您必须重新输入密钥。',
  'Hourly Bandwidth Limit': '每小时流量限制',
  'Not Set': '未设置',
  ' MBytes/hour': ' MB/小时',
  'Minimum space to leave on disk': '在磁盘上留下的最小空间',
  'If this value is set, the client will stop running if the free space on the disk decreases below this value.': '如果设置了此值，则如果磁盘上的可用空间减少到低于此值，客户端将停止运行。',
  'Note that even if this is not set, the client will exit if free space drops below 1 GB.': '请注意，即使未设置，如果可用空间低于 1GB，客户端也会退出。',
  'No Limit': '无限制',
  Scheduler: '运行计划',
  'The scheduler allows you to specify periods of time where the client is operating with lower speed limits.': '运行计划允许您指定客户端以较低速度限制运行的时间段。',
  'Click here to modify the schedule of this client': '修改此客户端的运行计划',
  'Static Ranges': '静态范围',
  'Over time, your client is assigned a number of static ranges that indicate specific, permanently assigned ranges of files it is able to serve. This toggle will reset this set of ranges.': '随着时间的推移，您的客户端将被分配数组静态范围，这些范围指示它能够提供服务的特定的、永久分配的多组文件。此开关将重置这些范围。',
  ...
  'Disable logging to disk. This will reduce disk activity by a small amount. Errors are still logged.': '停止向磁盘写入日志记录。这将少量减少磁盘活动。错误仍然被记录。',
  'Run in low-memory mode. This will somewhat reduce memory requirements, but will lead to increased disk activity.': '在低内存模式下运行。这会在一定程度上减少内存需求，但会导致磁盘活动增加。',
  'Use this client as your designated H@H Downloader. Only necessary if you have multiple clients. (H@H 1.3+ required)': '将此客户端用作您指定的 H@H 下载器。仅当您有多个客户端时才需要。（需要 H@H 1.3+）',
...
}, [[/^This is the maximum speed the client is allowed to use, measured in kilobytes per second\. Must be at least (\d+\.*\d*)\s*([KMGTP]B)\/s$/, '这是允许客户端使用的最大速度。必须至少为 $1$2/s。'], [/^This must be at least (\d+\.*\d*)\s*([KMGTP]B)\. We recommend at least 1 GB disk per 25 KByte\/s bandwidth, or 5 GB for every MBit\/s, but more is always better.$/, '至少为 $1$2。我们建议为每 25KB/s 带宽保留至少 1GB 磁盘，或每 Mbps 带宽 5GB 磁盘，但越多越好。'], [/The number of megabytes this client is allowed to send every hour. Leave at 0 to limit with max speed alone. Must be at least (\d+\.*\d*)\s*([KMGTP]B) if set./, '此客户端每小时允许发送的 MB 数。保留为 0 表示仅限制最大速度。如果设置，必须至少为 $1$2。'], [/^(\d+\.*\d*)\s*([KMGTP]B)\/hour$/, '$1 $2/小时']]);
merge(/^\/hentaiathome\.php\?.*act=schedule/, undefined, {
  'The scheduler allows you to specify periods of time where the client is operating with lower speed limits. The speed limit cannot be set higher than the default entry. To change the default entry, alter the Maximum Burst Speed on the Settings page.': '运行计划允许您指定客户端以较低速度限制运行的时间段。速度限制不能高于默认规则。要更改默认规则，请更改设置页面上的“最大并发速率”。',
  'To add a new scheduling entry, simply use the row at the bottom. Enter the required values and make sure that Active is checked, then hit Apply Schedule. Repeat to add more than one entry. You can have up to 9 entries per client, excluding the default entry. In case of overlap, the schedule with the highest granularity will always be applied.': '要添加新的规则，只需使用最后一行。输入所需的值并确保选中“启用”，然后点击“保存运行计划”。重复以上步骤以添加多个规则。在默认规则之外，每个客户端最多可以有 9 个规则。在重叠的情况下，将始终应用具有最高粒度的规则。',
  Active: '启用',
  Mon: '一',
  Tue: '二',
  Wed: '三',
...
}, [[/^Modify Schedule for Client (\d+) \((.*?)\)$/, '修改客户端 $1（$2）的运行计划'], [/^All schedule times are in UTC\. As a reference, the current UTC time is (.*?)\.$/, function (s, t) {
...
var data = {
  ...
  'Max 2,000 files and 10 GB per gallery. Do not upload more than 500MB at a time, less if you have a slow connection.': '每个图库最多包含 2000 张图片或 10GB。一次上传的大小不应超过 500MB，如果连接速度较慢，可以尝试以更小归档上传。',
  'Start Upload': '开始上传',
  ...
  _createClass(UiTranslation, [{
    ...
    value: function get() {
      ...
      dataMaps.filter(function (d) {
        var _d$hosts$includes, _d$hosts;

        return d.regex.test(path) && ((_d$hosts$includes = (_d$hosts = d.hosts) === null || _d$hosts === void 0 ? void 0 : _d$hosts.includes(location.hostname)) !== null && _d$hosts$includes !== void 0 ? _d$hosts$includes : true);
      }).forEach(function (d) {
...
var package_namespaceObject = JSON.parse('{"name":"ehsyringe","displayName":"EhSyringe","version":"2.8.4","description":"E 站注射器，将中文翻译注入到 E 站体内。","author":"EhTagTranslation","repository":{"type":"git","url":"git+https://github.com/EhTagTranslation/EhSyringe.git"},"license":"MIT","bugs":"https://github.com/EhTagTranslation/EhSyringe/issues","homepage":"https://github.com/EhTagTranslation/EhSyringe","type":"module","scripts":{"start:monkey":"webpack serve --mode=development --env type=user-script","start:ext":"webpack --mode=development --watch --env type=web-ext","start:chrome":"yarn start:ext --env vendor=chrome","start:firefox":"yarn start:ext --env vendor=firefox","build":"webpack --mode=production","build:monkey":"yarn build --env type=user-script","build:ext":"yarn build --env type=web-ext","build:chrome":"yarn build:ext --env vendor=chrome && web-ext build -s dist -a releases -n ehsyringe.chrome.zip -o","build:firefox":"yarn build:ext --env vendor=firefox && web-ext build -s dist -a releases -n ehsyringe.firefox.xpi -o","lint":"eslint ./src/**/*.ts","format":"prettier --ignore-path .gitignore --write .","clean":"rimraf dist releases"},"devDependencies":{"@babel/core":"^7.17.5","@babel/plugin-transform-runtime":"^7.17.0","@babel/preset-env":"^7.16.11","@types/chrome":"^0.0.179","@types/escape-html":"^1.0.1","@types/node":"^17.0.21","@types/tampermonkey":"^4.0.5","@types/webextension-polyfill":"^0.8.2","@typescript-eslint/eslint-plugin":"^5.13.0","@typescript-eslint/parser":"^5.13.0","babel-loader":"^8.2.3","copy-webpack-plugin":"^10.2.4","css-loader":"^6.6.0","cssnano":"^5.1.0","eslint":"^8.10.0","eslint-config-prettier":"^8.5.0","eslint-plugin-prettier":"^4.0.0","execa":"^6.1.0","html-webpack-plugin":"^5.5.0","less":"^4.1.2","less-loader":"^10.2.0","postcss":"^8.4.7","postcss-import":"^14.0.2","postcss-loader":"^6.2.1","postcss-preset-env":"^7.4.2","prettier":"^2.5.1","rimraf":"^3.0.2","semver":"^7.3.5","style-loader":"^3.3.1","ts-loader":"^9.2.7","tsconfig-paths-webpack-plugin":"^3.5.2","type-fest":"^2.12.0","typescript":"^4.6.2","web-ext":"^6.7.0","webpack":"^5.69.1","webpack-bundle-analyzer":"^4.5.0","webpack-cli":"^4.9.2","webpack-dev-server":"^4.7.4","webpack-userscript":"^2.5.8","webpack-webextension-plugin":"^1.0.0","yargs":"^17.3.1"},"dependencies":{"core-js":"^3.21.1","emoji-regex":"^10.0.0","escape-html":"^1.0.3","idb-keyval":"^6.1.0","lit-html":"^2.2.0","rxjs":"^7.5.4","tslib":"^2.3.1","typedi":"^0.10.0","webextension-polyfill":"^0.8.0"}}');
...
var Syringe = /*#__PURE__*/function () {
  ...
  _createClass(Syringe, [{
    ...
    value: function translateUi(node) {
      var _node$parentElement, _node$parentElement$p;
      ...
      if (isNode(node, 'a') && (node === null || node === void 0 ? void 0 : (_node$parentElement = node.parentElement) === null || _node$parentElement === void 0 ? void 0 : (_node$parentElement$p = _node$parentElement.parentElement) === null || _node$parentElement$p === void 0 ? void 0 : _node$parentElement$p.id) === 'nb') {
        var _node$textContent3;

        var _translation4 = this.translateUiText((_node$textContent3 = node.textContent) !== null && _node$textContent3 !== void 0 ? _node$textContent3 : '');
      ...
      if (isNode(node, 'p') && node.classList.contains('gpc')) {
        ...
        (_node$parentElement2 = node.parentElement) === null || _node$parentElement2 === void 0 ? void 0 : _node$parentElement2.insertBefore(p, node);
    ...
    value: function init() {
      ...
      body.addEventListener('contextmenu', function (ev) {
        ...
        if (node.nodeName === 'IMG') {
          ...
          _this.url = (_node$getAttribute = node.getAttribute('src')) !== null && _node$getAttribute !== void 0 ? _node$getAttribute : undefined;
          showMenu = matcher('image', _this.url);
        } else if (node.nodeName === 'A') {
          var _node$getAttribute2;

          _this.url = (_node$getAttribute2 = node.getAttribute('href')) !== null && _node$getAttribute2 !== void 0 ? _node$getAttribute2 : undefined;
...
var _notification = typeof GM_notification == 'function' ? GM_notification : function (detailsOrText, ondoneOrTitle, image, onclick) {
  ...
  (_notification$onclick = notification.onclick) === null || _notification$onclick === void 0 ? void 0 : _notification$onclick.call(notification);
  (_notification$ondone = notification.ondone) === null || _notification$ondone === void 0 ? void 0 : _notification$ondone.call(notification, true);
};
...
var TagContextMenu = /*#__PURE__*/function () {
  function TagContextMenu(tagging) {
    ...
    this.onclick = function (info) {
      var _info$url$split$pop$r, _info$url$split$pop, _seg$pop;
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              ...
              case 6:
                ...
                _context2.prev = 8;
                _context2.next = 11;
                return this.http.download(url, 'GET', function (ev) {
                  var total = ev.lengthComputable ? ev.total : asset != null ? asset.size : 0;
                  if (total > 0) progress === null || progress === void 0 ? void 0 : progress(ev.loaded / total);else progress === null || progress === void 0 ? void 0 : progress(0);
                }, 'json');

              case 11:
                result = _context2.sent;

                if (!((result === null || result === void 0 ? void 0 : (_result$head = result.head) === null || _result$head === void 0 ? void 0 : _result$head.sha) === version.target_commitish && result !== null && result !== void 0 && result.data)) {
                  _context2.next = 15;
                  break;
                }
...
function database_updater_ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }

function database_updater_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? database_updater_ownKeys(Object(source), !0).forEach(function (key) { _defineProperty(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : database_updater_ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }
...
function tag_database_ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }

function tag_database_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? tag_database_ownKeys(Object(source), !0).forEach(function (key) { _defineProperty(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : tag_database_ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }
...
var ImageContextMenu = /*#__PURE__*/function () {
  function ImageContextMenu() {
    ...
    this.onclick = function (info) {
      ...
      var url = (_info$url = info.url) !== null && _info$url !== void 0 ? _info$url : '';
...
function popup_ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }

function popup_objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? popup_ownKeys(Object(source), !0).forEach(function (key) { _defineProperty(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : popup_ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }
  ...
  _createClass(Popup, [{
    ...
    value: function () {
      var _checkVersion = _asyncToGenerator( /*#__PURE__*/regenerator_default().mark(function _callee3() {
        ...
        var currentSha, updateTime, data, hasNewData;
        return regenerator_default().wrap(function _callee3$(_context3) {
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
    ...
    value: function template() {
      var state = this.state;
      return $(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral([" <div id=\"eh-syringe-popup-root\">\n            ", " ", "\n        </div>"])), this.mainPanelTemplate(), state.configValue ? this.settingPanelTemplate() : w);
    }
  }, {
    key: "mount",
...
function shouldShowPopup() {
  if (/^\/mpv\//i.test(location.pathname) || location.pathname === '/archiver.php' || location.pathname === '/gallerytorrents.php' || location.pathname === '/gallerypopups.php') {
    return false;
  }
